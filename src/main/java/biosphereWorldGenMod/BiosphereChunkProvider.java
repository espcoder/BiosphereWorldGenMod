...
import biosphereWorldGenMod.Helpers.AvgCalc;
...
import biosphereWorldGenMod.Helpers.TopDownBoundingBox;
import biosphereWorldGenMod.Helpers.Utils;
import biosphereWorldGenMod.Models.Sphere;
import biosphereWorldGenMod.Models.SphereChunk;
			...
			for (int xo = 0; xo < 16; ++xo)
			{
				...
				for (int rawY = ModConsts.WORLD_MAX_Y; rawY >= ModConsts.WORLD_MIN_Y; rawY--)
				{
					...
					if (rawY > midY)
					{
						...
						{
                            if (!this.config.isBridgeEnabled())
                            {
                                if (rawY >= midY)
                                {
                            ...
                            else if (rawY >= midY + 4 || Math.abs(rawX - sphere.sphereLocation.posX) > bridgeWidth
					...
					{
						if (this.config.isFullDomeEnabled())
                        {
                            block = sphere.getDomeBlock(rawX, rawY, rawZ);
                        }
                        else
