...
import biosphereWorldGenMod.BiomeEntry;
import biosphereWorldGenMod.BiosphereWorldType;
import biosphereWorldGenMod.BlockEntry;
import biosphereWorldGenMod.Helpers.Blx;
import biosphereWorldGenMod.Helpers.Creator;
import biosphereWorldGenMod.Helpers.IKeyProvider;
import biosphereWorldGenMod.Helpers.LruCacheList;
import biosphereWorldGenMod.Helpers.ModConsts;
import biosphereWorldGenMod.Helpers.Predicate;
import biosphereWorldGenMod.Helpers.Utils;
...
public class ModConfig
{
	...
	private static final boolean defaultBridgeEnabled = true;
	private static final boolean defaultFullDomeEnabled = false;
    ...
    // #region Full Dome Enabled

    private boolean fullDomeEnabled = true;

    public boolean isFullDomeEnabled()
    {
        return this.fullDomeEnabled;
    }
    public void setFullDomeEnabled(boolean fullDomeEnabled)
    {
        this.fullDomeEnabled = fullDomeEnabled;
    }
    private static Property getFullDomeEnabledProperty()
    {
        if (cfgFile == null) {
            return null;
        }
        return cfgFile.get("general", "Full Dome Enabled", false, "Controls whether the spheres have a full dome.");
    }
    private BooleanWorldProperty getFullDomeEnabledWorldProperty()
    {
        return new BooleanWorldProperty(getFullDomeEnabledProperty(), Boolean.valueOf(isFullDomeEnabled()), Boolean.valueOf(true));
    }

    // #endregion

	// #region Bridge Enabled
	...
	private static Property getBridgeEnabledProperty()
	{
		if (cfgFile == null) {
			return null;
		}
		return cfgFile.get("general", "Bridge Enabled", true, "Controls whether the bridge between spheres is enabled.");
	}
	private BooleanWorldProperty getBridgeEnabledWorldProperty()
	{
		return new BooleanWorldProperty(getBridgeEnabledProperty(), Boolean.valueOf(isBridgeEnabled()), Boolean.valueOf(true));
	}
	...
	private void LoadConfigurationFromWorld()
	{
        ...
        setFullDomeEnabled(((Boolean)getFullDomeEnabledWorldProperty().ReadWorldValue(data)).booleanValue());
		setBridgeEnabled(((Boolean)getBridgeEnabledWorldProperty().ReadWorldValue(data)).booleanValue());
	...
	private void SaveConfigurationToWorld()
	{
        ...
        getFullDomeEnabledWorldProperty().WriteWorldValue(data);
		getBridgeEnabledWorldProperty().WriteWorldValue(data);
	...
	private void LoadConfigurationFromFile()
	{
        ...
        this.setFullDomeEnabled(getFullDomeEnabledProperty().getBoolean());
		this.setBridgeEnabled(getBridgeEnabledProperty().getBoolean());
	...
	private void SaveConfigurationToFile()
	{
        ...
        getFullDomeEnabledProperty().set(isFullDomeEnabled());
		getBridgeEnabledProperty().set(isBridgeEnabled());
